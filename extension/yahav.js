...
await chrome.storage.local.set({pendingYahav:true,
  ...
  syncStatus:'יהב: בודק את החיבור'});const tab=await yahavTab();if(!tab){await chrome.storage.local.set({syncStatus:'ממתין להתחברות ליהב — הזן קוד משתמש, ת״ז וסיסמה במסך שנפתח'});await openLoginWindow({url:YAHAV_LOGIN,type:'popup',width:560,height:780,focused:true});return{ok:true,status:'waiting_login'}}await chrome.storage.local.set({pendingYahav:false,syncStatus:'יהב: מרענן את החיבור'});await returnToDashboard(tab.id,true);await chrome.tabs.reload(tab.id);await delay(3000);await chrome.storage.local.set({pendingYahav:true,syncStatus:'יהב: החיבור מוכן, מתחיל סנכרון'});runYahav(tab.id).catch(async e=>{await chrome.storage.local.set({pendingYahav:false,syncStatus:`שגיאה ביהב: ${e.message}`});await chrome.runtime.openOptionsPage()});return{ok:true,status:'syncing'}}
