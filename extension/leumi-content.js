...
async function selectAccount(key){const current=parseAccount(txt(chooser()));if(current?.key===key)return;const compactPage=normalized(txt(document.body));if(compactPage.includes(key))return;let option=accountTabs().find(o=>parseAccount(txt(o))?.key===key);if(!option){const opts=await options();option=opts.find(o=>parseAccount(txt(o))?.key===key)}if(!option)throw Error(`החשבון ${key} לא נמצא בחיבור לאומי`);realClick(option);for(let i=0;i<60;i++){await wait(250);const active=parseAccount(txt(chooser()));if(active?.key===key||normalized(txt(document.body)).includes(key))return}throw Error(`לאומי לא עבר לחשבון ${key}`)}
