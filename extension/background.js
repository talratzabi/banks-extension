...
// ריצה נקייה עם פירוט מחזירה את התקרה המלאה (המפסק הוריד אותה בחצי אחרי timeout).
if(ctl.enable&&!probe.error&&probe.enriched>0&&ctl.cap<MIZRAHI_DETAIL_CAP)try{const st=(await chrome.storage.local.get({mizrahiDetailState:{}})).mizrahiDetailState||{};await chrome.storage.local.set({mizrahiDetailState:{...st,cap:MIZRAHI_DETAIL_CAP}})}catch{}
