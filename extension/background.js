...
await chrome.storage.local.set({leumiGap:Object.keys(gaps).length?{at:now,accounts:gaps}:null,leumiRangeProbe:r.rangeProbe||null,leumiDateMenu:r.dateMenu||null,leumiRadios:r.radios||null});const txCount=result.reduce((sum,a)=>sum+(a.transactions?.length||0),0),loanCount=result.reduce((sum,a)=>sum+(a.loans?.length||0),0),chequeCount=result.reduce((sum,a)=>sum+(a.chequeCount||0),0);
