...
await chrome.storage.local.set({pendingLeumi:true,leumiAttempts:0,leumiOptionProbe:null,discoveredAccounts:prev.discoveredAccounts.filter(a=>a.source!=='leumi'),syncStatus:'טוען את החיבור הפעיל ללאומי ומזהה חשבונות'});const tabs=await chrome.tabs.query({url:['https://hb2.bankleumi.co.il/*']});if(tabs.length){const tab=leumiTab(tabs);await chrome.tabs.update(tab.id,{active:true});await chrome.tabs.reload(tab.id);return{ok:true,status:'discovering'}}await chrome.storage.local.set({syncStatus:'ממתין להתחברות ללאומי'});await chrome.tabs.create({url:'https://www.leumi.co.il/',active:true});return{ok:true,status:'waiting_login'}}
...
const ping=await chrome.tabs.sendMessage(tabId,{type:'LEUMI_PING'});if(ping?.ok)return}
...
async function leumiSnapshot(tabId){try{const s=await chrome.tabs.sendMessage(tabId,{type:'LEUMI_SNAPSHOT'});return s?.debug||null}catch{return null}}
...
async function harvestLeumiCheques(tabId,accounts,txUrl){const have=await chequeKeys();let saved=0,routed=false;
...
if(!wanted.length)continue;if(!routed){await prepareLeumiRoute(tabId,txUrl);routed=true}
...
try{r=await chrome.tabs.sendMessage(tabId,{type:'LEUMI_CHEQUE_IMAGES',wanted:batch,key:a.key})}catch{}
if(!r?.ok)continue;
...
async function runMax(tabId,requestedSuffix=''){
 ...
 try{
  ...
  await delay(1700);await prepareMax(tabId);const he=['ינואר','פברואר','מרץ','אפריל','מאי','יוני','יולי','אוגוסט','ספטמבר','אוקטובר','נובמבר','דצמבר'],monthsFromPicker=(await chrome.tabs.sendMessage(tabId,{type:'MAX_MONTHS'}).catch(()=>null))?.labels||[],calendarMonths=Array.from({length:12},(x,i)=>{const d=new Date();d.setDate(1);d.setMonth(d.getMonth()-i);return`${he[d.getMonth()]} ${d.getFullYear()}`}),months=(monthsFromPicker.length?monthsFromPicker:calendarMonths).map(label=>{const p=String(label).split(/\s+/),i=he.indexOf(p[0]),y=Number(p[1]);return i<0||!y?null:{label,key:`${String(i+1).padStart(2,'0')}.${y}`,rank:y*12+i}}).filter(Boolean).sort((a,b)=>b.rank-a.rank).slice(0,12),wanted=String(requestedSuffix||'').replace(/\D/g,'').slice(-4),monthly=[],seen=new Set();
  if(!months.length)throw Error('רשימת חודשי החיוב של MAX לא נקראה');
  for(let i=0;i<months.length;i++){const {label,key}=months[i];await chrome.storage.local.set({syncStatus:`MAX: טוען חודש ${i+1} מתוך ${months.length} · ${key}`});const sel=await chrome.tabs.sendMessage(tabId,{type:'MAX_SELECT_MONTH',label});if(!sel?.ok){if(i===0)throw Error(sel?.error||'בורר החודשים לא נטען');break}let page=null,candidate='',stable=0;for(let w=0;w<30;w++){await delay(350);await prepareMax(tabId);try{page=await chrome.tabs.sendMessage(tabId,{type:'MAX_READ'})}catch{}const ready=page?.ok&&page.month===key&&page.fingerprint;if(ready&&candidate===page.fingerprint)stable++;else{candidate=ready?page.fingerprint:'';stable=ready?1:0}if(stable>=3)break}if(!page?.ok||page.month!==key||stable<3)throw Error(`דף ${key} לא התייצב לקריאה`);if(seen.has(key))throw Error(`MAX נשאר בחודש ${key}`);seen.add(key);const groups=Object.entries(page.cards||{}).filter(([suffix])=>suffix&&suffix!=='unknown'&&(!wanted||suffix===wanted)),cards=groups.map(([suffix,transactions])=>({suffix,name:'כרטיס MAX',issuer:'MAX',amount:transactions.reduce((s,t)=>s+Math.abs(Number(t.amount)||0),0),transactions,month:key}));if(wanted&&!cards.length&&page.transactions?.length)throw Error(`כרטיס ${wanted} לא נמצא בדף ${key}`);if(cards.length){await storeCardMonth(key,cards);monthly.push(...cards)}
  }
  if(!monthly.length)throw Error('לא נקראו עסקאות חודשיות מ‑MAX');const latestBySuffix=new Map();for(const c of monthly)if(!latestBySuffix.has(c.suffix))latestBySuffix.set(c.suffix,c);const nowLabel=months[0].label,nowKey=months[0].key;await chrome.storage.local.set({syncStatus:`MAX: החיוב הקרוב נקבע ל-${nowLabel}${monthsFromPicker.length?'':' (לפי לוח השנה — הבורר לא נקרא)'}`});await chrome.tabs.sendMessage(tabId,{type:'MAX_SELECT_MONTH',label:nowLabel});await delay(1800);await prepareMax(tabId);let currentPage=await chrome.tabs.sendMessage(tabId,{type:'MAX_READ'});if(!currentPage?.ok||currentPage.month!==nowKey)currentPage={ok:true,month:nowKey,total:0,cards:{}};const details=[...latestBySuffix.keys()].map(suffix=>{const transactions=currentPage.cards?.[suffix]||[],onlyCard=latestBySuffix.size===1,amount=onlyCard&&Number.isFinite(Number(currentPage.total))?Number(currentPage.total):transactions.reduce((s,t)=>s+Math.abs(Number(t.amount)||0),0);return{...latestBySuffix.get(suffix),amount,transactions,month:currentPage.month}}),state=await chrome.storage.local.get({accounts:[],maxLastCards:[]}),accounts=state.accounts.map(a=>({...a,cards:[...(a.cards||[])]})),unassigned=[];let assigned=0;const digits=v=>String(v||'').replace(/\D/g,'');
