...
try{if(!window.__bfcacheGuard){window.__bfcacheGuard=1;
  ...
  window.addEventListener('pageshow',e=>{if(e.persisted){
    try{chrome.storage.local.set({bfcacheSeen:{source:'cal',at:new Date().toISOString(),
      url:String(location.href).slice(0,140)}})}catch(err){}}});
}}catch(e){}

// שומר הזרקה עמיד למות הקשר — ראה discount-content.js
if(window.__calSyncLoaded){try{if(window.__calSyncLoaded())return}catch(e){}}
// ⚠ ההפניה נתפסת כאן ולא נקראת מחדש בכל בדיקה: קריאה מחדש
// מחזירה את ה-chrome החדש, והגשש מדווח „חי" גם כשההקשר שלו מת. נתפס בבדיקה.
const __rt__calSyncLoaded=(()=>{try{return chrome.runtime}catch(e){return null}})();
window.__calSyncLoaded=()=>{try{return !!(__rt__calSyncLoaded&&__rt__calSyncLoaded.id)}catch(e){return false}};
...
function authenticated(){return location.hostname==='digital-web.cal-online.co.il'&&/יציאה/.test(document.body?.innerText||'')}
function report(){if(authenticated())chrome.runtime.sendMessage({type:'CAL_AUTHENTICATED'}).catch(()=>{})}
report();setInterval(report,5000);
function home(){const text=clean(document.body?.innerText),owner=(text.match(/היי\s+([^,]+),/)||[])[1]||'',bank=(text.match(/([\d]{5,12})\s*-\s*([\d]{1,3})/)||[]),card=(text.match(/(?:^|\s)(\d{4})\s+[^\n]{0,80}?חיוב קרוב\s+(\d{2}\/\d{2}\/\d{2,4})\s+₪?\s*([\d,.\-]+)/)||[]);return{owner:clean(owner),debitAccount:bank[1]||'',debitCode:bank[2]||'',suffix:card[1]||((text.match(/(?:דיינרס|ויזה|מאסטרקארד)[^\d]{0,30}(\d{4})/)||[])[1]||''),chargeDate:card[2]||((text.match(/חיוב קרוב\s+(\d{2}\/\d{2}\/\d{2,4})/)||[])[1]||''),amount:card[3]?money(card[3]):money((text.match(/חיוב קרוב[\s\S]{0,80}?₪\s*([\d,.\-]+)/)||[])[1])}}
function activeCards(){return[...document.querySelectorAll('[role="checkbox"].card-btn')].filter(b=>!/without-all$/.test(b.id||'')).map(b=>{const text=clean(b.innerText),suffix=(text.match(/(\d{4})$/)||[])[1];return suffix?{suffix,name:text.replace(/\s*\d{4}$/,'')}:null}).filter(Boolean)}
