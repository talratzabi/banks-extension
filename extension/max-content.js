...
// ⚠ 28.08.2026 - "בורר החודשים של MAX לא נמצא": הממשק החדש שבר גם את
// .combo.dates. לא מנחשים סלקטור - הגשש רושם את המועמדים ל-storage והתיקון
// ייכתב מהמדידה. נרשמים רק שמות מחלקות, תפקידים ותוויות חודשים - בלי סכומים
// ובלי שמות בתי עסק.
function uiProbe(){
  try{
    const cap=(s,n)=>String(s||'').replace(/\s+/g,' ').trim().slice(0,n);
    const cls=e=>cap(e.getAttribute&&e.getAttribute('class'),90);
    const monthRe=/(ינואר|פברואר|מרץ|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר)\s+\d{4}/;
    const cands=[...document.querySelectorAll('body *')].filter(e=>{
      const c=cls(e),t=cap(e.textContent,60);
      return /combo|month|picker|dropdown|date|select/i.test(c)||(t.length<60&&monthRe.test(t));
    }).slice(0,90).map(e=>({t:e.tagName,c:cls(e),m:(cap(e.textContent,120).match(monthRe)||[''])[0],r:e.getAttribute('role')||'',a:cap(e.getAttribute('aria-label'),40)}));
    const rowsInv={};
    for(const e of document.querySelectorAll('[class]')){const c=cls(e);if(/row|cell|item|transaction|table|list/i.test(c))rowsInv[c]=(rowsInv[c]||0)+1}
    chrome.storage.local.set({maxUiProbe:{at:new Date().toISOString(),path:cap(location.pathname,60),
      oldCombo:!!document.querySelector('.combo.dates'),oldRows:document.querySelectorAll('.row.body').length,
      cands,rows:Object.entries(rowsInv).sort((a,b)=>b[1]-a[1]).slice(0,30)}});
  }catch(e){try{chrome.storage.local.set({maxUiProbe:{error:String(e&&e.message)}})}catch(_){}}
}
if(/transaction-details/.test(location.pathname))setTimeout(uiProbe,3000);
