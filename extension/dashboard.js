...
async function openChequeAccounts(){
 ...
 let stored=0;try{stored=(await chequeKeys()).size}catch(e){}
 const missing=stored-Object.keys(chequeAccounts).length;
 box.innerHTML=`<p class="muted">${nChq(stored)} שמורים · ${total} מהם שויכו ל-${groups.length} חשבונות`
  +`${unread.length?` · ${unread.length} נקראו ולא זוהה חשבון`:''}`
  +`${missing>0?` · <b>${nChq(missing)} עדיין לא נקראו</b>`:''}.</p>`
  +`<div class="acct-bar">${missing>0?`<button type="button" id="readAccountsNow">קרא את החשבון של ${nChq(missing)}</button>`:''}`
  ...
  +(groups.length?`<div class="acct-table"><table><thead><tr><th>בנק</th><th>סניף</th><th>חשבון</th><th>שיקים</th><th>שם המוסר</th><th></th></tr></thead><tbody>`
   ...
   +`</tbody></table></div>`
   :`<div class="empty">${stored?'עדיין לא נקרא אף מספר חשבון.':'אין צילומי שיקים שמורים.'}</div>`);
}
// הקריאה עצמה - מופעלת מתוך החלון, ומדווחת בתוכו.
async function readChequeAccounts(){
 const btn=$('#readAccountsNow');if(btn)btn.disabled=true;
 ...
 try{
  const r=await chequeAccountsAll({
