...
async function renderMovementSearch(){
 ...
 if(!q&&minRaw===''&&maxRaw===''&&!fromRaw&&!toRaw&&flowPick==='both'){box.className='empty';box.textContent='הקלד שם, סכום או תקופה — התוצאות יופיעו מיד.';return}
 ...
 const matched=rows.filter(r=>(!q||r.name.toLocaleLowerCase('he').includes(q))&&r.amount>=min&&r.amount<=max&&r.when>=from&&r.when<=to&&(flowPick==='both'||r.flow===flowPick)).sort((a,b)=>b.when-a.when),filtered=matched.slice(0,500);
