...
function lastTwelveMonths(){const out=[],d=new Date();for(let i=0;i<12;i++){out.push(`${String(d.getMonth()+1).padStart(2,'0')}${d.getFullYear()}`);d.setMonth(d.getMonth()-1)}return out}
...
box.before(tools);
chrome.storage.local.get({autoSyncOnLogin:true}).then(x=>{for(const c of document.querySelectorAll('[id="autoSyncOnLogin"]'))c.checked=x.autoSyncOnLogin});
