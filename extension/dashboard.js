...
let accounts=[],discovered=[],selectedKeys=[],syncScope='business',accountFilter='both',accountKinds={},privateOwnerName='',hideMortgages=false,isracardUnassigned=[],calUnassigned=[],maxUnassigned=[],isracardLastCards=[],calLastCards=[],maxLastCards=[],cardHistoryStats={},cardNewMarks={},activeView='accounts',loadEpoch=0,loadTimer=null,monthlyCardMonths=null,monthlyPick='';
...
async function load(){
  ...
  const data=await chrome.storage.local.get({viewSince:'',viewSinceInit:false,collectSince:'',accounts:[],discoveredAccounts:[],selectedAccountKeys:null,syncScope:'business',accountFilter:'both',accountKinds:{},privateOwnerName:'',hideMortgages:false,isracardUnassigned:[],calUnassigned:[],maxUnassigned:[],isracardLastCards:[],calLastCards:[],maxLastCards:[],cardNewMarks:{},fibiConnectionNames:{},syncStatus:'טרם בוצע',syncProgress:null,statusBySource:{},bankDiagnostics:{},hiddenCards:[],maslaka:null,realEstate:[],balanceAssets:[],balanceLiabs:[],loanKinds:{}});
...
function movementGroups(matched,by){
 ...
 for(const r of matched){
  let key;
  if(by==='month'){if(!r.when)key='ללא תאריך';else{const d=new Date(r.when);key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}}
  else key=r.source;
  const cur=g.get(key)||{key,n:0,debit:0,credit:0};
 ...
 // ⚠ נמדד בבדיקה: 'ללא תאריך' עוקף כל YYYY-MM במיון היורד (אותיות אחרי
 // ספרות) והתיישב בראש הטבלה. הדלי חסר-התאריך נדחף לסוף במפורש.
 if(by==='month')return list.sort((a,b)=>a.key==='ללא תאריך'?1:b.key==='ללא תאריך'?-1:String(b.key).localeCompare(String(a.key)));
 return list.sort((a,b)=>(b.debit+b.credit)-(a.debit+a.credit));
}
function paintMovementBreakdown(){
 ...
 if(movementBreakdown==='source'&&!manySources)movementBreakdown='none';
 const btn=(id,label)=>`<button type="button" data-bd="${id}" class="${movementBreakdown===id?'on':''}">${label}</button>`;
 const bar=`<div class="movement-breakdown-bar"><span>פילוח הסיכום:</span>${btn('none','ללא')}${btn('month','לפי חודש')}${manySources?btn('source','לפי מקור'):''}</div>`;
 if(movementBreakdown==='none'){box.innerHTML=bar;return}
 const rows=movementGroups(matched,movementBreakdown),
  tn=rows.reduce((x,r)=>x+r.n,0),td=rows.reduce((x,r)=>x+r.debit,0),tc=rows.reduce((x,r)=>x+r.credit,0);
 box.innerHTML=bar+`<div class="movement-breakdown-table"><table><thead><tr><th>${movementBreakdown==='month'?'חודש':'מקור'}</th><th>תנועות</th><th>חובה</th><th>זכות</th><th>נטו</th></tr></thead><tbody>`
  +rows.map(r=>`<tr><td>${esc(movementBreakdown==='month'?movementMonthTitle(r.key):r.key)}</td><td>${r.n}</td><td class="bd-debit">${money(r.debit)}</td><td class="bd-credit">${money(r.credit)}</td><td>${money(r.debit-r.credit)}</td></tr>`).join('')
  +`<tr class="bd-total"><th>סה״כ ${rows.length} ${movementBreakdown==='month'?'חודשים':'מקורות'}</th><th>${tn}</th><th>${money(td)}</th><th>${money(tc)}</th><th>${money(td-tc)}</th></tr></tbody></table></div>`;
}
...
async function renderMovementSearch(){
 const box=$('#movementSearchResults');if(!box)return;const epoch=++movementSearchEpoch,q=String($('#movementSearchText')?.value||'').trim().toLocaleLowerCase('he'),minRaw=$('#movementSearchMin')?.value,maxRaw=$('#movementSearchMax')?.value,fromRaw=$('#movementSearchFrom')?.value,toRaw=$('#movementSearchTo')?.value,flowPick=String($('#movementSearchKind')?.value||'both');
 ...
 const matched=rows.filter(r=>(!q||r.name.toLocaleLowerCase('he').includes(q))&&r.amount>=min&&r.amount<=max&&r.when>=from&&r.when<=to&&(flowPick==='both'||r.flow===flowPick)&&(!movementSourcePick.size||movementSourcePick.has(r.source))).sort((a,b)=>b.when-a.when),filtered=matched.slice(0,500);
 ...
 if(!matched.length){box.className='empty';box.textContent='לא נמצאו תנועות שמתאימות לחיפוש.';return}const capped=matched.length>filtered.length;box.className='movement-search-table';box.innerHTML=`<div class="movement-search-summary"><span>נמצאו ${matched.length} תנועות</span><span>סכום מצטבר ${money(sum)}</span></div><table><thead><tr><th>תאריך</th><th>ספק / לקוח / פעולה</th><th>מקור</th><th>סוג</th><th>סכום</th></tr></thead><tbody>${filtered.map(r=>`<tr><td>${esc(r.date||'')}</td><td><b>${esc(r.name||'ללא פירוט')}</b>${r.fresh?' <span class="new-tag">חדש</span>':''}</td><td>${esc(r.source)}</td><td>${esc(r.kind)}</td><td>${money(r.amount)}</td></tr>`).join('')}</tbody><tfoot><tr><th colspan="4">סה״כ ${matched.length} תנועות</th><th>${money(sum)}</th></tr></tfoot></table><div class="movement-search-total"><span class="mst-debit">חובה · ${debitRows.length} תנועות · ${money(debitSum)}</span><span class="mst-credit">זכות · ${creditRows.length} תנועות · ${money(creditSum)}</span><span>נטו (חובה פחות זכות) · ${money(debitSum-creditSum)}</span></div><div id="movementBreakdownBox"></div>${capped?`<div class="movement-search-note">מוצגות ${filtered.length} התנועות האחרונות מתוך ${matched.length}; הסיכום כולל את כולן.</div>`:''}`;
...
$('#movementSearchResults').addEventListener('click',e=>{const b=e.target.closest('button[data-bd]');if(!b)return;movementBreakdown=b.dataset.bd;paintMovementBreakdown()});
