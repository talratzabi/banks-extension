...
// ⚠ 03.09.2026 - טל: "מתחת לשם הכרטיס: זוהה חיוב אחרון בחשבון זה וזה ואת
// תאריך הזיהוי." chargeMatch נכתב ברקע (findLastCharge) - כאן רק תצוגה.
function chargeMatchLine(c){const m=c?.chargeMatch;if(!m)return'';const a=accounts.find(x=>x.id===m.accountId);const label=a?`${a.sourceLabel||''} ${a.branch}-${a.accountNumber}`:'';
  return`<small class="charge-match">זוהה חיוב אחרון בחשבון ${esc(label)} · ${esc(m.date)} · ${money(m.amount)}${m.text?` · ${esc(m.text)}`:''} · זוהה ב-${esc(shortDateTime(m.at))}</small>`}
...
function pendingDiscovered(){
 ...
 // ⚠⚠ 31.08.2026 - טל: "בא לסנכרן דיסקונט פרטי, לא נפתחים החשבונות".
 // **הסינון מ-1.91.0 בלע אותם:** חשבון שסונכרן פעם הוסר מהבורר, ולכן
 // כשטל התחבר שוב לאותו בנק כדי לסנכרן מחדש - הרשימה נראתה ריקה.
 // **זיהוי טרי גובר על "כבר שמור":** אם המשתמש בדיוק התחבר לבנק וביקש
 // לזהות, החשבונות שלו חייבים להופיע - גם אם הם כבר במערכת.
 const focus=chooserFocus&&chooserFocus.source;
 return discovered.filter(a=>{
  if(focus&&a.source===focus)return true;
...
// ⚠⚠ 31.08.2026 - טל: "אני מסנכרן בינלאומי ורואה חשבונות של לאומי?"
// צודק. חשבונות שזוהו ומעולם לא סונכרנו נשארים "ממתינים" לנצח, ולכן הם
// הופיעו גם בזמן ריצה של בנק אחר. הסינון מ-1.91.0 טיפל בשמורים בלבד.
// **הפתרון: חותם זמן על כל רשומת זיהוי** (נוסף ב-background בארבעת
// מסלולי הזיהוי), והבורר מציג את **הבנק שזוהה אחרון** פתוח, ואת השאר
// בסעיף מקופל. ⚠ שום דבר לא נמחק ולא מוסתר - רק מסודר לפי מה שרלוונטי
// עכשיו, כי גם "לאבד בחירה ממתינה" היה באג (זה בדיוק 1.11.2).
// ⚠ אבחון הבורר: מה יש באחסון מול מה מוצג. אחרי שלושה סבבים שבהם ניחשתי
// במקום למדוד, זה הכלי שמחליף את הניחוש הבא.
async function copyChooserAudit(){
 const st=await chrome.storage.local.get({discoveredAccounts:[],chooserFocus:null,selectedAccountKeys:null,
   discountPrivateOptions:null,discountPrivateTriggerProbe:null,pendingSources:[],syncStatus:''});
 const split=splitDiscovered();
 const bySource=o=>{const m={};for(const a of o||[])m[a.source||'?']=(m[a.source||'?']||0)+1;return m};
 const text=JSON.stringify({
  גרסה:chrome.runtime.getManifest().version,
  סטטוס:st.syncStatus,
  מיקוד:st.chooserFocus,
  זוהו_באחסון:bySource(st.discoveredAccounts),
  זוהו_מפתחות:(st.discoveredAccounts||[]).map(a=>({key:a.key,source:a.source,at:a.at||0,ready:!!(a.branch&&a.accountNumber)})),
  חשבונות_שמורים:bySource(accounts),
  שמורים_מפתחות:accounts.map(a=>a.selectionKey||`${a.source||'business'}|${a.branch}-${a.accountNumber}`),
  אחרי_סינון:bySource(pendingDiscovered()),
  מוצג_בפועל:bySource(split.current),
  נפילה_לאחור:split.fallback||null,
  נבחרו:(st.selectedAccountKeys||[]).length,
  דיסקונט_גשש:st.discountPrivateOptions,
  דיסקונט_פותחן:st.discountPrivateTriggerProbe},null,1);
 try{await navigator.clipboard.writeText(text);toast('אבחון הבחירה הועתק')}
 catch(e){toast('ההעתקה נכשלה — האבחון בקונסול');console.log(text)}
}
document.addEventListener('click',e=>{if(e.target.closest('#chooserAudit'))copyChooserAudit()});
function splitDiscovered(){
 const list=pendingDiscovered();
 if(!list.length)return{current:[],older:[],label:''};
 const at=a=>Number(a.at)||0;
 // ⚠⚠ **המיקוד מנצח תמיד, גם כשאין לו חשבונות.** בגרסה הקודמת מיקוד על
 // בנק בלי ממתינים נפל אחורה לחותם הזמן הגדול - והציג את לאומי, שזו
 // בדיוק התלונה של טל. אם התחברת לבנק ואין בו מה לבחור, התשובה היא
 // "אין", ולא חשבונות של בנק אחר.
 const focus=chooserFocus&&chooserFocus.source;
 let src=focus||'';
 if(!src){
  // בלי מיקוד כלל (נתונים מלפני הגרסה) - נפילה לאחור לחותם הזמן.
  const newest=Math.max(...list.map(at));
  src=newest>0?(list.find(a=>at(a)===newest)||{}).source||'':'';
 }
...
if(!discovered.length){box.innerHTML=`<div class="empty">אין חשבונות שממתינים לבחירה מהבנק האחרון שהתחברת אליו.${split.older.length?` <b>${split.older.length} חשבונות מבנקים אחרים ממתינים</b> — הם יופיעו כאן ברגע שתתחבר לאותו בנק.`:''} הסנכרון לא מתחיל לפני שתאשר.</div>`;
  ...
  if(tools)tools.innerHTML='<button type="button" class="button" id="probeTab" title="קורא את מבנה הדף בלשונית הפעילה. קריאה בלבד — בלי לחיצות ובלי ניווט.">מדוד לשונית פעילה</button>'
   +'<button type="button" class="button" id="chooserAudit" title="מעתיק את מצב הבורר: מה זוהה, מאיזה בנק, מה כבר שמור, ומה המיקוד הנוכחי">העתקת אבחון בחירה</button>';
  return}
