...
function splitDiscovered(){
 ...
 // רשומה בלי חותם היא מסבב ישן (לפני הגרסה הזאת), ולכן נחשבת הישנה ביותר.
 const at=a=>Number(a.at)||0;
 const newest=Math.max(...list.map(at));
 const src=(list.find(a=>at(a)===newest)||{}).source;
 // כל הרשומות של אותו בנק נכנסות יחד - זיהוי אחד מייצר אותן באותו רגע.
 const current=list.filter(a=>a.source===src&&newest>0);
 const older=list.filter(a=>!current.includes(a));
 return{current:current.length?current:list,older:current.length?older:[],
  label:(list.find(a=>a.source===src)||{}).sourceLabel||''};
}
...
const totalPending=discovered.length+split.older.length;
if(tab)tab.textContent=totalPending?`בחירה וסנכרון (${totalPending})`:'בחירה וסנכרון';
...
box.innerHTML=discovered.map(choiceRow).join('')
 +(split.older.length?`<details class="older-choices"><summary>חשבונות שזוהו קודם בבנקים אחרים (${split.older.length})</summary>`
   +split.older.map(choiceRow).join('')+`</details>`:'');box.onchange=async e=>{const c=e.target.closest('input[type=checkbox]');if(c){selectedKeys=c.checked?[...new Set([...selectedKeys,c.value])]:selectedKeys.filter(k=>k!==c.value);await chrome.storage.local.set({selectedAccountKeys:selectedKeys})}updateSelectionCount()};updateSelectionCount()}
