...
let accounts=[],discovered=[],selectedKeys=[],syncScope='business',accountFilter='both',accountKinds={},privateOwnerName='',hideMortgages=false,isracardUnassigned=[],calUnassigned=[],maxUnassigned=[],isracardLastCards=[],calLastCards=[],maxLastCards=[],cardHistoryStats={},cardNewMarks={},chequeInfo={},chequePayers={},chequePayerDoubt={},activeView='accounts',loadEpoch=0,loadTimer=null,monthlyCardMonths=null,monthlyPick='';
...
async function load(){
  ...
  const data=await chrome.storage.local.get({viewSince:'',viewSinceInit:false,collectSince:'',accounts:[],discoveredAccounts:[],selectedAccountKeys:null,syncScope:'business',accountFilter:'both',accountKinds:{},privateOwnerName:'',hideMortgages:false,isracardUnassigned:[],calUnassigned:[],maxUnassigned:[],isracardLastCards:[],calLastCards:[],maxLastCards:[],cardNewMarks:{},chequeInfo:{},chequePayers:{},chequePayerDoubt:{},fibiConnectionNames:{},syncStatus:'טרם בוצע',syncProgress:null,statusBySource:{},bankDiagnostics:{},hiddenCards:[],maslaka:null,realEstate:[],balanceAssets:[],balanceLiabs:[],loanKinds:{}});
...
function chequePayerHtml(id){
 ...
 if(name)return ` <button type="button" class="payer-tag" data-payer="${esc(id)}" title="שם מוסר השיק. לחיצה לתיקון.">${esc(name)}</button>`;
 const d=chequePayerDoubt[id];
 if(!d)return '';
 const opts=[d.crop,d.full].filter(Boolean).join(' / ')||'לא נקרא';
...
async function editChequePayer(id){
 ...
 const hint=d?`שתי הקריאות לא הסכימו:${nl}· ${d.crop||'(ריק)'}${nl}· ${d.full||'(ריק)'}${nl}${nl}`:'';
...
document.addEventListener('click',e=>{const p=e.target.closest('[data-payer]');if(p)editChequePayer(p.dataset.payer)});
...
$('#ocrCheques')?.addEventListener('click',async()=>{
 ...
 try{
  ...
  toast(r.total?`${r.found} שמות הוכרעו, ${r.unsure||0} נשארו בספק — לחיצה על התגית מתקנת`:'כל הצילומים כבר נקראו');
