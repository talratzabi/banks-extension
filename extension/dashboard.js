...
const statusStyles=document.createElement('style');statusStyles.textContent='.sync-detail{display:block;overflow-wrap:anywhere;word-break:break-word;max-height:5.5em;overflow-y:auto}';document.head.appendChild(statusStyles);
...
function renderSyncStatus(raw=''){const value=String(raw||''),el=$('#syncStatus');let banner=$('#syncBanner');if(!banner){banner=document.createElement('section');banner.id='syncBanner';banner.className='panel';document.querySelector('.summary')?.before(banner)}let state='waiting',label='ממתין לחיבור';if(/שגיאה|נכשל/.test(value)){state='error';label='הסנכרון נכשל'}else if(/לפני פחות מ-|האוטומטי כבוי|תור הסנכרון|כבר רץ|לא נשמרו חשבונות|אין חיבור שאושר/.test(value)){state='waiting';label='הסנכרון דולג'}else if(/הסתיים|סונכרן|כבר עודכן/.test(value)){state='done';label=/הסתיים/.test(value)?'הסנכרון הסתיים בהצלחה':'הסנכרון הסתיים'}else if(/קורא|מסנכרן|מזהה|מחפש|בודק|טוען|מתבצע|שומר|שולף|מוריד|פותח|מעדכן|ממתין להתחברות/.test(value)){state='running';label='סנכרון בתהליך'}// ⚠⚠ 22.08.2026 — טל: „התוסף לא תקין", עם צילום של מסך כמעט ריק ובו
// מחרוזת אחת ארוכה וגלילה אופקית. **התוסף היה תקין לגמרי.** מה שנשבר
// הוא הפריסה: הודעת הכשל של לאומי מכילה את כתובת הלשונית, ובלאומי היא
// כוללת פרמטר `?q=` דחוס באורך אלף תווים. מחרוזת אחת בלי רווחים אינה
// נשברת לשורות, ולכן היא מתחה את הדף לרוחב והסתירה את כל השאר.
// שני הגנות, כי אחת לא מספיקה: **קיצור** לתצוגה (האבחון המלא ממילא
// נשמר במפתחות הייעודיים — leumiDebug, leumiDiscoverError), ו-CSS
// שמאפשר שבירה בתוך מילה כך ש**שום** מחרוזת עתידית לא תוכל לעשות זאת שוב.
const shown=String(value||'').length>320?String(value).slice(0,320)+'…':value;
