...
async function renderMovementSearch(){
 ...
 for(const a of accounts.filter(a=>accountFilter==='both'||kindOf(a)===accountFilter))for(const t of visibleTx(a)){const amount=Math.abs(Number(t.debit)||Number(t.credit)||Number(t.amount)||0),name=`${t.action||''} ${t.details||''}`.trim(),when=dateKey(t.date);const chqId=a.source==='leumi'&&t.cheque&&t.reference?chequeId(accountKey(a),t.reference):'';
  ...
  rows.push({date:t.date,when,name,payer,hay:`${name} ${payer} ${payerBank}`.toLocaleLowerCase('he'),amount,cheque:chqId&&chequeIds.has(chqId)?chqId:'',source:`${a.sourceLabel||'בנק'} · ${a.branch}-${a.accountNumber}`,type:'bank',flow:Number(t.credit)?'credit':'debit',kind:Number(t.credit)?'זכות':'חובה'})}
...
async function openChequeAccounts(){
 ...
 box.innerHTML=`<p class="muted">${total} שיקים ב-${groups.length} חשבונות · ${unread.length} שיקים שהחשבון שלהם עדיין לא נקרא.</p>`
  +(groups.length?`<div class="acct-table"><table><thead><tr><th>בנק</th><th>סניף</th><th>חשבון</th><th>שיקים</th><th>שם המוסר</th><th></th></tr></thead><tbody>`
   +groups.map(g=>`<tr data-acct="${esc(g.key)}"><td>${esc(g.bank)}</td><td>${esc(g.branch)}</td><td>${esc(g.account)}</td>`
   ...
   +`</tbody></table></div>`:'<div class="empty">עדיין לא נקרא אף מספר חשבון. לחץ "שיקים לפי חשבון מוסר" כדי לקרוא.</div>');
}
...
$('#chequeAccounts')?.addEventListener('click',async()=>{
 const btn=$('#chequeAccounts'),state=$('#ocrChequesState');
 const avail=await chequeOcrAvailability();
 if(avail==='unsupported'||avail==='unavailable'){await openChequeAccounts();return toast('המודל אינו זמין — מוצג מה שכבר נקרא')}
 const missing=[...await chequeKeys()].filter(id=>!chequeAccounts[id]).length;
 if(missing&&!confirm([`${missing} שיקים עדיין בלי מספר חשבון.`,
   'לקרוא אותם עכשיו משורת הספרות שבתחתית השיק?','הכול מקומי.'].join(String.fromCharCode(10)))){
  await openChequeAccounts();return;
 }
 btn.disabled=true;
 try{
  if(missing){const r=await chequeAccountsAll({
    onDownload:l=>{state.textContent=`מוריד מודל… ${Math.round(l*100)}%`},
    onProgress:({done,total,read})=>{state.textContent=`קורא חשבונות ${done}/${total} · הוכרעו ${read}`}});
   chequeAccounts=(await chrome.storage.local.get({chequeAccounts:{}})).chequeAccounts||{};
   state.textContent=`הסתיים — ${r.read} מתוך ${r.total}`;}
  await openChequeAccounts();
 }catch(e){state.textContent='';toast(`הקריאה נכשלה: ${e.message}`)}
