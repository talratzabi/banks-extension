...
let collectSince='';
chrome.storage.local.get({collectSince:''}).then(x=>{collectSince=String(x.collectSince||'')});
document.addEventListener('change',async e=>{const input=e.target.closest('#collectSince');if(!input)return;
  collectSince=String(input.value||'');
  await chrome.storage.local.set({collectSince});
  toast(collectSince?`איסוף הנתונים יתחיל מ-${collectSince}`:'הוסר גבול תחילת האיסוף — ייאסף כל מה שהבנק מציע');
...
if(probe){probe.disabled=true;probe.textContent='מודד…';const r=await chrome.runtime.sendMessage({type:'PROBE_ACTIVE_TAB'});probe.disabled=false;probe.textContent='מדוד לשונית פעילה';toast(r?.ok?`נמדד: ${r.host}`:(r?.error||'המדידה נכשלה'));return}
