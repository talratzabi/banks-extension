...
let accounts=[],discovered=[],selectedKeys=[],syncScope='business',accountFilter='both',accountKinds={},privateOwnerName='',hideMortgages=false,isracardUnassigned=[],calUnassigned=[],maxUnassigned=[],isracardLastCards=[],calLastCards=[],maxLastCards=[],cardHistoryStats={},cardNewMarks={},chequeInfo={},activeView='accounts',loadEpoch=0,loadTimer=null,monthlyCardMonths=null,monthlyPick='';
...
async function load(){
  ...
  const data=await chrome.storage.local.get({viewSince:'',viewSinceInit:false,collectSince:'',accounts:[],discoveredAccounts:[],selectedAccountKeys:null,syncScope:'business',accountFilter:'both',accountKinds:{},privateOwnerName:'',hideMortgages:false,isracardUnassigned:[],calUnassigned:[],maxUnassigned:[],isracardLastCards:[],calLastCards:[],maxLastCards:[],cardNewMarks:{},chequeInfo:{},fibiConnectionNames:{},syncStatus:'טרם בוצע',syncProgress:null,statusBySource:{},bankDiagnostics:{},hiddenCards:[],maslaka:null,realEstate:[],balanceAssets:[],balanceLiabs:[],loanKinds:{}});
...
async function renderMovementSearch(){
 ...
 for(const a of accounts.filter(a=>accountFilter==='both'||kindOf(a)===accountFilter))for(const t of visibleTx(a)){const amount=Math.abs(Number(t.debit)||Number(t.credit)||Number(t.amount)||0),name=`${t.action||''} ${t.details||''}`.trim(),when=dateKey(t.date);const chqId=a.source==='leumi'&&t.cheque&&t.reference?chequeId(accountKey(a),t.reference):'';
  rows.push({date:t.date,when,name,amount,cheque:chqId&&chequeIds.has(chqId)?chqId:'',source:`${a.sourceLabel||'בנק'} · ${a.branch}-${a.accountNumber}`,type:'bank',flow:Number(t.credit)?'credit':'debit',kind:Number(t.credit)?'זכות':'חובה'})}
...
$('#movementSearchResults').addEventListener('click',async e=>{
 ...
 if(record?.front)showCheque(record,{date:chq.dataset.date});else toast('צילום השיק אינו במסד המקומי');
});
...
body.innerHTML=`<div class="cheque-shots"><figure><figcaption>קדמי</figcaption><img alt="צילום תמונת שיק מלפנים" src="${esc(record.front)}"></figure>${record.back?`<figure><figcaption>אחורי</figcaption><img alt="צילום תמונת שיק מאחור" src="${esc(record.back)}"></figure>`:''}</div>${record.info?`<p class="cheque-note-block"><b>פרטים מחלון השיק בלאומי:</b> ${esc(record.info)}</p>`:''}<small class="sync-detail">נשמר מקומית ${record.savedAt?new Date(record.savedAt).toLocaleString('he-IL'):''} — נפתח גם ללא חיבור לבנק.</small>`;
