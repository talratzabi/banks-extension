...
let accounts=[],discovered=[],selectedKeys=[],syncScope='business',accountFilter='both',accountKinds={},privateOwnerName='',hideMortgages=false,isracardUnassigned=[],calUnassigned=[],maxUnassigned=[],isracardLastCards=[],calLastCards=[],maxLastCards=[],cardHistoryStats={},cardNewMarks={},chequeInfo={},chequePayers={},activeView='accounts',loadEpoch=0,loadTimer=null,monthlyCardMonths=null,monthlyPick='';
...
async function load(){
  ...
  const data=await chrome.storage.local.get({viewSince:'',viewSinceInit:false,collectSince:'',accounts:[],discoveredAccounts:[],selectedAccountKeys:null,syncScope:'business',accountFilter:'both',accountKinds:{},privateOwnerName:'',hideMortgages:false,isracardUnassigned:[],calUnassigned:[],maxUnassigned:[],isracardLastCards:[],calLastCards:[],maxLastCards:[],cardNewMarks:{},chequeInfo:{},chequePayers:{},fibiConnectionNames:{},syncStatus:'טרם בוצע',syncProgress:null,statusBySource:{},bankDiagnostics:{},hiddenCards:[],maslaka:null,realEstate:[],balanceAssets:[],balanceLiabs:[],loanKinds:{}});
...
function chequePayerHtml(id){
 ...
 return name?` <span class="payer-tag">מוסר: ${esc(name)}</span>`:'';
}
...
async function renderMovementSearch(){
 ...
 for(const a of accounts.filter(a=>accountFilter==='both'||kindOf(a)===accountFilter))for(const t of visibleTx(a)){const amount=Math.abs(Number(t.debit)||Number(t.credit)||Number(t.amount)||0),name=`${t.action||''} ${t.details||''}`.trim(),when=dateKey(t.date);const chqId=a.source==='leumi'&&t.cheque&&t.reference?chequeId(accountKey(a),t.reference):'';
  const payer=chqId?String(chequePayers[chqId]||''):'';
  rows.push({date:t.date,when,name,payer,hay:`${name} ${payer}`.toLocaleLowerCase('he'),amount,cheque:chqId&&chequeIds.has(chqId)?chqId:'',source:`${a.sourceLabel||'בנק'} · ${a.branch}-${a.accountNumber}`,type:'bank',flow:Number(t.credit)?'credit':'debit',kind:Number(t.credit)?'זכות':'חובה'})}
...
$('#chequeModalTitle').textContent=`צילום שיק · ${data.date||''} · אסמכתא ${record.reference}${chequePayers[record.id]?` · מוסר: ${chequePayers[record.id]}`:''}`;
...
$('#ocrCheques')?.addEventListener('click',async()=>{
 ...
 try{
  const r=await chequeOcrRunAll({
   ...
   onProgress:({done,total,found})=>{state.textContent=`קורא שיקים ${done}/${total} · זוהו ${found} שמות`}});
  chequePayers=(await chrome.storage.local.get({chequePayers:{}})).chequePayers||{};
  state.textContent=r.total?`הסתיים — ${r.found} שמות מתוך ${r.total} צילומים`:'אין צילומים חדשים לקריאה';
  toast(r.total?`זוהו ${r.found} שמות מוסרים מתוך ${r.total}`:'כל הצילומים כבר נקראו');
