...
function chequePayerHtml(id){
 ...
 if(name){
  const how=m.stamp?`אושר מול החותמת (${m.stamp})`:m.kind==='private'?'בלי חותמת — שיק פרטי; שתי קריאות הסכימו':'לחיצה לתיקון';
  return ` <button type="button" class="payer-tag${m.stamp?' stamped':''}" data-payer="${esc(id)}" title="${esc(how)}. לחיצה לתיקון.">${esc(name)}</button>`;
 }
...
async function paintPayerReview(){
 ...
 save.onclick=async()=>{
   ...
   payerQueue.splice(payerAt,1);await paintPayerReview();render();scheduleMovementSearch();
 };
 ...
 box.append(count,img,cbox,input,acts);
