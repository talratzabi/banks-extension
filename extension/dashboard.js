...
async function renderMovementSearch(){
 ...
 const matched=rows.filter(r=>(!terms.length||terms.some(term=>r.name.toLocaleLowerCase('he').includes(term)))&&r.amount>=min&&r.amount<=max&&r.when>=from&&r.when<=to&&(flowPick==='both'||r.flow===flowPick)&&(!movementSourcePick.size||movementSourcePick.has(r.source))).sort((a,b)=>b.when-a.when),filtered=matched.slice(0,500);
 ...
 if(!matched.length){box.className='empty';box.textContent='לא נמצאו תנועות שמתאימות לחיפוש.';return}const capped=matched.length>filtered.length;box.className='movement-search-table';const perTerm=terms.length>1?terms.map(term=>({term,n:matched.filter(r=>r.name.toLocaleLowerCase('he').includes(term)).length})):[];
