...
const movementSourcePick=new Set();let movementSourceSig='';
const BANK_BUTTONS=[
...
function renderMovementSources(rows){
 ...
 if(sig!==movementSourceSig){movementSourceSig=sig;
  list.innerHTML=items.length?`<div class="msrc-actions"><button type="button" data-act="all">כל המקורות</button><button type="button" data-act="banks">בנקים בלבד</button><button type="button" data-act="cards">כרטיסים בלבד</button></div>`
   +items.map(i=>`<label><input type="checkbox" data-src="${esc(i.source)}" data-type="${i.type}"${movementSourcePick.has(i.source)?' checked':''}><span>${esc(i.source)}</span><span class="msrc-tag">${i.type==='bank'?'בנק':'כרטיס'} · ${i.n}</span></label>`).join('')
   :'<div class="msrc-empty">אין עדיין תנועות שמורות — סנכרן בנק או כרטיס.</div>'}
 else for(const box of list.querySelectorAll('input[data-src]'))box.checked=movementSourcePick.has(box.dataset.src);
...
$('#movementSourceList').addEventListener('change',e=>{const box=e.target.closest('input[data-src]');if(!box)return;if(box.checked)movementSourcePick.add(box.dataset.src);else movementSourcePick.delete(box.dataset.src)});
